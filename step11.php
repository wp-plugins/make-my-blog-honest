...
if (!class_exists('MakeMyBlogHonest')) {
	...
	class MakeMyBlogHonest {
		...
		function DealConfigurationPageJs() {
			...
			wp_register_script(
					self::PREFIX . '_deal_configuration_page', $this->plugin_url . '/js/deal_configuration_page.js', array('jquery'), '1.0', true
			);
		...
		function DealConfigurationPageCss() {
			...
			wp_register_style(
					self::PREFIX . '_deal_configuration_page', $this->plugin_url . '/css/deal_configuration_page.css', false, '1.0', 'screen'
			);
		...
		private function AddDeal($product_name, $sale_price, $enabled = false, $expires = false) {
			...
			if (empty($product_name)) {
				...
				$this->output['error'] = __('The product name cannot be empty', self::PREFIX);
			...
			if (!is_numeric($sale_price)) {
				...
				$this->output['error'] = __('The sale price must be a valid number.', self::PREFIX);
			...
			if (is_array($expires)) {
				...
				if (
					...
					$this->output['error'] =
						__('A year month and day are required for the expiration date.', self::PREFIX);
			...
			$sql = $wpdb->prepare(
				...
				' VALUES(%s,%s, %s, %s);', $product_name, $sale_price, $enabled, $expires
			);
			...
			if ($result !== false) {
				...
				$this->output['error'] = __(
					'There was a problem adding the deal, please try again.', self::PREFIX);
		...
		private function SetEnabledStatus($id, $enabled) {
			...
			if ($id == false) {
				...
				$this->output['error'] = __(
					'There was a problem changing the deal status, please try again.', self::PREFIX);
			...
			$sql = $wpdb->prepare(
				...
				'_deals SET `enabled` = %d WHERE `id` = %d;', $enabled, $id
			);
			...
			if ($result !== false) {
				...
				$this->output['success'] = __('The enabled status has been updated.', self::PREFIX);
		...
		private function DeleteDeal($id = false) {
			...
			if ($id == false) {
				...
				$this->output['error'] = __(
					'There was a problem deleting the deal, please try again.', self::PREFIX);
			...
			if ($result !== false) {
				...
				$this->output['success'] = __('The deal has been deleted.', self::PREFIX);
	...
	register_activation_hook(
		'makemybloghonest/makemybloghonest.php', 'MakeMyBlogHonest_Activate'
	);

	register_deactivation_hook(
		'makemybloghonest/makemybloghonest.php', 'MakeMyBlogHonest_Deactivate'
	);
